...
def launch_setup(context, *args, **kwargs):
    ...
    rviz_config_file = PathJoinSubstitution(
        [FindPackageShare(description_package.perform(context)), "launch", "view_robot.rviz"]
    )
    ...
    robot_controllers = PathJoinSubstitution([FindPackageShare(runtime_config_package.perform(context)), controllers_file.perform(context)])
    ...
    nodes = [robot_state_publisher_node, rviz_node, control_node, robotiq_force_torque_sensor_broadcaster_spawner]
...
def generate_launch_description():
    ...
    declared_arguments.append(DeclareLaunchArgument("runtime_config_package", default_value="robotiq_ft_sensor_hardware"))
    declared_arguments.append(DeclareLaunchArgument("controllers_file", default_value="config/robotiq_controllers.yaml"))
    declared_arguments.append(DeclareLaunchArgument("description_package", default_value="robotiq_ft_sensor_description"))
    declared_arguments.append(DeclareLaunchArgument("description_file", default_value="urdf/example_use_robotiq_ft300.urdf.xacro"))
    ...
    declared_arguments.append(DeclareLaunchArgument("use_fake_mode", default_value="false"))
    declared_arguments.append(DeclareLaunchArgument("use_add_fts_wrench", default_value="true"))
    declared_arguments.append(DeclareLaunchArgument("add_fts_wrench_topic", default_value="add_fts_wrench"))
